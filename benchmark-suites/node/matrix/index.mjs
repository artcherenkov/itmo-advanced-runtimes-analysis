/**
 * Бенчмарк умножения матриц для Node.js - ESM версия
 */
import path from 'path';
import { fileURLToPath } from 'url';
import { benchmark } from '../utils/metrics.mjs';
import { preventNumericOptimization } from '../utils/deoptimize.mjs';
...
async function runBenchmark() {
  ...
  console.log(`Запуск бенчмарка умножения матриц для Node.js (${process.version})`);
  ...
  const benchmarkOptions = {
    ...
    outputPath: path.join(__dirname, '../../../results', `node_matrix_${algorithm}_size${matrixSize}_${Date.now()}.json`)
  };
