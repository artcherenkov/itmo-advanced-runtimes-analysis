...
# Скрипт для запуска вычислительных тестов Node.js
# Автоматически запускает выбранные тесты для Node.js:
...
function show_help {
  ...
  echo "Опции:"
  echo "  -a, --all            Запустить все вычислительные тесты для Node.js"
  ...
  echo "Примеры:"
  echo "  $0 --all             Запустить все вычислительные тесты для Node.js"
  echo "  $0 -t fibonacci      Запустить тест Фибоначчи для Node.js"
...
IMAGE_NAME="node-computational-benchmark"
CONTAINER_NAME_BASE="node-computational-benchmark"
...
run_single_test() {
  ...
  echo -e "\n\033[1;34m=== Запуск теста: node_${test_type} ===\033[0m"
  
  # Собираем Docker-образ
  echo "Сборка образа для теста $test_type..."
  docker build -t $IMAGE_NAME -f ./benchmark-suites/node/computational/Dockerfile .
  ...
  if [ $exit_code -ne 0 ]; then
    echo -e "\033[1;31mОшибка при выполнении теста node_${test_type} (код $exit_code)\033[0m"
  else
    echo -e "\033[1;32mТест node_${test_type} успешно завершен\033[0m"
  fi
...
echo -e "\033[1;33mБудут последовательно выполнены следующие тесты для Node.js:\033[0m"
