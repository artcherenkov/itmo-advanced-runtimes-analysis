...
# Скрипт для измерения времени холодного старта Node.js в Docker-контейнере
# Измеряемые метрики:
...
CONTAINER_NAME_BASE="node-cold-start-benchmark"

# Имя образа
IMAGE_NAME="node-cold-start-benchmark"

# Имя файла с результатами и начальная структура JSON
RESULTS_FILE="results/cold-start/node_cold_start_benchmark_$(date +%Y%m%d_%H%M%S).json"
...
# Функция для получения информации о среде Node.js в контейнере
get_node_environment() {
    log "Сбор информации о среде выполнения Node.js..."
    
    # Запускаем временный контейнер для сбора информации
    local container_id=$(docker run -d --name node-env-info-$(date +%s) $IMAGE_NAME sleep 15)
    ...
    # Получаем настройки Node.js
    local node_config=$(docker exec $container_id node -e "console.log(JSON.stringify({
    ...
    # Возвращаем информацию в формате JSON
    echo "{\"os\":\"$os_info\",\"memory_limit\":\"$memory_limit\",\"cpu_shares\":\"$cpu_shares\",\"node_config\":$node_config}"
}
...
build_image() {
    ...
    docker build -t $IMAGE_NAME -f benchmark-suites/node/cold-start/Dockerfile .
    log "Docker-образ собран успешно"
}

# Функция для извлечения версии Node.js
get_node_version() {
    local version=$(docker run --rm $IMAGE_NAME node -v | tr -d '\n\r')
    log "Определена версия Node.js: $version"
    echo "$version"
}
...
calculate_statistics() {
    ...
    # Получаем версию Node.js напрямую для JSON
    local node_version=$(docker run --rm $IMAGE_NAME node -v | tr -d '\n\r')
    log "Определена версия Node.js для результатов: $node_version"
...
# Главная функция
main() {
    log "=== Начало тестирования холодного старта Node.js v$SCRIPT_VERSION ==="
    ...
    init_results_file "$node_version"
    ...
    for ((i=1; i<=$ITERATIONS; i++)); do
        ...
        if ! run_benchmark_iteration $i "$node_version"; then
    ...
    log "=== Тестирование холодного старта Node.js завершено ==="
    log "Результаты сохранены в файл: $RESULTS_FILE"
}
