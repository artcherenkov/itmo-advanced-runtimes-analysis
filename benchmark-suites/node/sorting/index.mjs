/**
 * Бенчмарк алгоритма сортировки для Node.js - ESM версия
 */
import path from 'path';
import { fileURLToPath } from 'url';
import { benchmark } from '../utils/metrics.mjs';
import { preventOptimization, preventNumericOptimization } from '../utils/deoptimize.mjs';
...
async function runBenchmark() {
  ...
  const arraySize = parseInt(process.env.ARRAY_SIZE) || 10000;
  const algorithm = process.env.SORT_ALGORITHM || 'quicksort';
  const iterations = parseInt(process.env.ITERATIONS) || 30;
  
  console.log(`Запуск бенчмарка сортировки для Node.js (${process.version})`);
  ...
  const benchmarkOptions = {
    ...
    outputPath: path.join(__dirname, '../../../results', `node_sorting_${algorithm}_size${arraySize}_${Date.now()}.json`)
  };
