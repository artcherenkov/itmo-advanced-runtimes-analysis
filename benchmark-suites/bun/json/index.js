...
function generateLargeJsonObject(size) {
  const result = {
    ...
    metadata: {
      ...
      version: "1.0.0",
      tags: ["benchmark", "json", "bun", "javascript"],
      settings: {
        enabled: true,
        timeout: 5000,
        retries: 3,
...
function runBenchmark() {
  ...
  const objectSize = parseInt(args[0] || process.env.JSON_OBJECT_SIZE) || 1000;
  const operation = args[1] || process.env.JSON_OPERATION || 'parse-stringify';
  ...
  const benchmarkOptions = {
    ...
    outputPath: process.env.CONTAINER === 'true' 
      ? `/app/results/bun_json_${operation}_size${objectSize}_${Date.now()}.json`
      : join(import.meta.dir, '../../../results', `bun_json_${operation}_size${objectSize}_${Date.now()}.json`)
  };
  
  // Запускаем бенчмарк
  const results = benchmark(fnToTest, [testObject], benchmarkOptions);
