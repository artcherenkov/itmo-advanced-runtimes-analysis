...
import { preventNumericOptimization } from '../utils/deoptimize.js';
...
function runBenchmark() {
  ...
  const matrixSize = parseInt(args[0] || process.env.MATRIX_SIZE) || 250;
  const algorithm = args[1] || process.env.MATRIX_ALGORITHM || 'naive';
  const iterations = parseInt(args[2] || process.env.ITERATIONS) || 20;
  ...
  const benchmarkOptions = {
    ...
    outputPath: process.env.CONTAINER === 'true' 
      ? `/app/results/bun_matrix_${algorithm}_size${matrixSize}_${Date.now()}.json`
      : join(import.meta.dir, '../../../results', `bun_matrix_${algorithm}_size${matrixSize}_${Date.now()}.json`)
  };
  
  // Запускаем бенчмарк
  const results = benchmark(fnToTest, [matrixA, matrixB], benchmarkOptions);
