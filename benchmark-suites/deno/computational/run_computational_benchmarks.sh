...
# Скрипт для запуска вычислительных тестов Deno
# Автоматически запускает выбранные тесты для Deno:
...
function show_help {
  ...
  echo "Опции:"
  echo "  -a, --all            Запустить все вычислительные тесты для Deno"
  ...
  echo "Примеры:"
  echo "  $0 --all             Запустить все вычислительные тесты для Deno"
  echo "  $0 -t fibonacci      Запустить тест Фибоначчи для Deno"
...
IMAGE_NAME="deno-computational-benchmark"
CONTAINER_NAME_BASE="deno-computational-benchmark"
...
run_single_test() {
  ...
  echo -e "\n\033[1;34m=== Запуск теста: deno_${test_type} ===\033[0m"
  
  # Собираем Docker-образ
  echo "Сборка образа для теста $test_type..."
  docker build -t $IMAGE_NAME -f ./benchmark-suites/deno/computational/Dockerfile .
  ...
  if [ $exit_code -ne 0 ]; then
    echo -e "\033[1;31mОшибка при выполнении теста deno_${test_type} (код $exit_code)\033[0m"
  else
    echo -e "\033[1;32mТест deno_${test_type} успешно завершен\033[0m"
  fi
...
echo -e "\033[1;33mБудут последовательно выполнены следующие тесты для Deno:\033[0m"
