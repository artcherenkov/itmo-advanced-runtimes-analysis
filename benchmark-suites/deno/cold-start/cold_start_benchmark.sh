...
# Скрипт для измерения времени холодного старта Deno в Docker-контейнере
# Измеряемые метрики:
...
CONTAINER_NAME_BASE="deno-cold-start-benchmark"
...
# Функция для получения информации о среде Deno в контейнере
get_deno_environment() {
    log "Сбор информации о среде выполнения Deno..."
    
    # Запускаем временный контейнер для сбора информации
    local container_id=$(docker run -d --name deno-env-info-$(date +%s) $IMAGE_NAME sleep 15)
    ...
    # Получаем настройки Deno
    local deno_config=$(docker exec $container_id deno --version 2>/dev/null)
    ...
    # Возвращаем информацию в формате JSON
    echo "{\"os\":\"$os_info\",\"memory_limit\":\"$memory_limit\",\"cpu_shares\":\"$cpu_shares\",\"deno_version\":\"$deno_config\"}"
}
...
build_image() {
    ...
    docker build -t $IMAGE_NAME -f benchmark-suites/deno/cold-start/Dockerfile .
    log "Docker-образ собран успешно"
}

# Функция для извлечения версии Deno
get_deno_version() {
    local version=$(docker run --rm $IMAGE_NAME deno --version | head -n 1 | cut -d ' ' -f 2 | tr -d '\n\r')
    log "Определена версия Deno: $version"
    echo "$version"
}
...
calculate_statistics() {
    ...
    # Получаем версию Deno напрямую для JSON
    local deno_version=$(docker run --rm $IMAGE_NAME deno --version | head -n 1 | cut -d ' ' -f 2 | tr -d '\n\r')
    log "Определена версия Deno для результатов: $deno_version"
...
main() {
    ...
    init_results_file "$deno_version"
    ...
    for ((i=1; i<=$ITERATIONS; i++)); do
        ...
        if ! run_benchmark_iteration $i "$deno_version"; then
    ...
    log "=== Тестирование холодного старта Deno завершено ==="
    log "Результаты сохранены в файл: $RESULTS_FILE"
}
